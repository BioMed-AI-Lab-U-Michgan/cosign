...
from .cunet import ControlledUnetModel, ControlNet
import numpy as np

NUM_CLASSES = 1000


def cc_train_defaults():
    return dict(
        unet_path="",
...
def create_model_and_diffusion(
    ...
    if control:
        model_args["model_type"] = "control"
        control_net = create_model(**model_args)
        model_args["model_type"] = "controlled_unet"
        controlled_unet = create_model(**model_args)
        return control_net, controlled_unet, diffusion
    else:
        model_args["model_type"] = "unet"
...
def create_model(
    ...
    if model_type == "unet":
        ...
        model = ControlNet
    elif model_type == "controlled_unet":
